...
class StrategyAnalyzer():
    ...
    # async def _query_exchange(command, *args, **kwargs, depth=1, max_depth=3):
    #     """
    #     Attempts to query exchange for data max_depth times. If fail returns none.
    #     """
    #     if depth == max_depth:
    #         # failed to get data
    #         return None
    ...
    #         return await self._query_exchange(command, **kwargs, depth=depth+1)
    ...
    async def analyze_breakout(self, market_pair, exchange, breakout_threshold=0.75, 
        ...
        is_breaking_out = breakout_analyzer.is_breaking_out(
            breakout_value, breakout_threshold)
    ...
    async def analyze_rsi(self, market_pair, exchange, overbought_threshold=0, 
        ...
        rsi_data = {'value': rsi_value, 'is_overbought': is_overbought, 
    ...
    async def analyze_moving_averages(self, market_pair, exchange, sma_threshold=0, 
        ...
        ma_data = {'sma_value': sma_value, 'ema_value': ema_value, 'is_sma_trending': 
            is_sma_trending, 'is_ema_trending': is_ema_trending}
    ...
    async def analyze_ichimoku_cloud(self, market_pair, exchange, ichimoku_threshold=0):
        ...
        conversion_line_data = await self.__exchange_interface.get_historical_data(
            market_pair=market_pair,
            exchange=exchange,
            period_count=9,
            time_unit='1d'
        ...
        span_b_data = await self.__exchange_interface.get_historical_data(
            market_pair=market_pair,
            exchange=exchange,
            period_count=52,
            time_unit='1d'
        ...
        ichimoku_data = {'span_a_value': leading_span_a, 'span_b_value': leading_span_b, 
