...
class Klinger_oscillator(IndicatorUtils):
    def analyze(
        ...
        klinger_values = pandas.DataFrame(
            klinger_columns, index=dataframe.index
        )

        klinger_values["mean"] = dataframe[["high", "low", "close"]].mean(
            axis=1
        )
        for index in range(1, (klinger_values.shape[0])):
            klinger_values["dm"] = (
                dataframe["high"][index] - dataframe["low"][index]
            )
            if (
                klinger_values["mean"][index]
                > klinger_values["mean"][index - 1]
            ):
        ...
        for index in range(0, (klinger_values.shape[0])):
            klinger_values["cm_a"] = (
                klinger_values["dm"][index] + klinger_values["dm"][index - 1]
            )
            if (
                klinger_values["trend"][index]
                == klinger_values["trend"][index - 1]
            ):
                klinger_values["cm"][index] = (
                    klinger_values["cm"][index - 1]
                    + klinger_values["dm"][index]
                )
            else:
                klinger_values["cm"][index] = (
                    klinger_values["dm"][index]
        ...
        klinger_values["vf"] = (
            dataframe["volume"]
            * abs(2 * ((klinger_values["dm"] / klinger_values["cm"]) - 1))
            * klinger_values["trend"]
            * 100
        )
        klinger_values["vf_ema_short"] = (
            klinger_values["vf"]
            .ewm(
        ...
        klinger_values["kvo"] = (
            klinger_values["vf_ema_short"] - klinger_values["vf_ema_long"]
        )
        klinger_values["kvo_signal"] = (
            klinger_values["kvo"]
            .ewm(
                span=signal_period, min_periods=0, adjust=False, ignore_na=True
            )
            .mean()
        )
        ...
        for index in range(1, klinger_values.shape[0]):
            ...
            if (
                klinger_values["kvo_signal"][index] > 0
                and klinger_values["mean"][index]
                > klinger_values["mean"][index - 1]
            ):
