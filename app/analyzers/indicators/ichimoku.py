...
class Ichimoku(IndicatorUtils):
    def analyze(
        ...
        ichimoku_values = pandas.DataFrame(
            ichimoku_columns, index=dataframe.index
        )
        # value calculations
        ...
        low_senkou = (
            dataframe["low"].rolling(window=senkou_span_b_period).min()
        )
        high_tenkansen = (
            dataframe["high"].rolling(window=tenkansen_period).max()
        )
        high_kijunsen = dataframe["high"].rolling(window=kijunsen_period).max()
        high_senkou = (
            dataframe["high"].rolling(window=senkou_span_b_period).max()
        )
        ...
        try:
            ...
            ichimoku_values["leading_span_a"] = ichimoku_values[
                "leading_span_a"
            ].shift(cloud_displacement)
            ichimoku_values["leading_span_b"] = ichimoku_values[
                "leading_span_b"
            ].shift(cloud_displacement)
            ...
            if chart == None:
                if custom_strategy == None:
                    ...
                    if tk_cross_enabled:
                        tk_cross_hot = (
                            ichimoku_values["tenkansen"][date]
                            > ichimoku_values["kijunsen"][date]
                        )
                        tk_cross_cold = (
                            ichimoku_values["tenkansen"][date]
                            < ichimoku_values["kijunsen"][date]
                    ...
                    if leading_span_enabled:
                        leading_span_hot = (
                            ...
                            > ichimoku_values["leading_span_b"][
                                leading_span_date
                            ]
                        )
                        leading_span_cold = (
                    ...
                    if hot_thresh:
                        ichimoku_values.at[date, "is_hot"] = (
                            tk_cross_hot or leading_span_hot
                        )

                    if cold_thresh:
                        ichimoku_values.at[date, "is_cold"] = (
                            tk_cross_cold or leading_span_cold
                        )
                else:
