...
class StochRSICross(IndicatorUtils):
    ...
    def analyze(self, historical_data, period_count=14, signal=['stoch_rsi'], smooth_k = 10, smooth_d = 3, hot_thresh=None, cold_thresh=None):
        ...
        kd_values = pandas.DataFrame([rsi, stochrsi, stochrsi_K, stochrsi_D]).T.rename(columns={0: "rsi", 1: "stoch_rsi", 2: "k", 3: "d"}).copy()

        kd_values['stoch_rsi'] = kd_values['stoch_rsi'].multiply(100)
        kd_values['k'] = kd_values['k'].multiply(100)
        kd_values['d'] = kd_values['d'].multiply(100)

        stoch_cross = pandas.concat([dataframe, kd_values], axis=1)
        stoch_cross.dropna(how='all', inplace=True)

        previous_k, previous_d = stoch_cross.iloc[-2]['k'], stoch_cross.iloc[-2]['d']
        current_k, current_d = stoch_cross.iloc[-1]['k'], stoch_cross.iloc[-1]['d']
