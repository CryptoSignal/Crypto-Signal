...
class StochRSICross(IndicatorUtils):
    def analyze(
        ...
        kd_values = (
            pandas.DataFrame([rsi, stochrsi, stochrsi_K, stochrsi_D])
            .T.rename(
                columns={
                    0: "rsi",
                    1: "stoch_rsi",
        ...
        previous_k, previous_d = (
            stoch_cross.iloc[-2]["smooth_k"],
            stoch_cross.iloc[-2]["smooth_d"],
        )
        current_k, current_d = (
            stoch_cross.iloc[-1]["smooth_k"],
            stoch_cross.iloc[-1]["smooth_d"],
        )

        stoch_cross["is_hot"] = False
        stoch_cross["is_cold"] = False

        stoch_cross.at[stoch_cross.index[-1], "is_cold"] = (
            previous_k > previous_d and current_k < current_d
        )
        stoch_cross.at[stoch_cross.index[-1], "is_hot"] = (
            previous_k < previous_d and current_k > current_d
        )
