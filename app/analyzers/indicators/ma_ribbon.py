...
class MARibbon(IndicatorUtils):
    ...
    def MA_RIBBON(self, df, ma_series):
        ...
        corr_ts = pandas.Series(
            corr * 100, index=df.index, name="MARIBBON_CORR"
        ).round(2)
        pval_ts = pandas.Series(
            pval * 100, index=df.index, name="MARIBBON_PVAL"
        ).round(2)
        dist_ts = pandas.Series(dist, index=df.index, name="MARIBBON_DIST")

        return pandas.concat(
            [corr_ts, pval_ts, dist_ts] + ema_list, join="outer", axis=1
        )
    ...
    def analyze(
        ...
        ma_ribbon.rename(
            columns={"MARIBBON_PVAL": "pval", "MARIBBON_CORR": "corr"},
            inplace=True,
        )
