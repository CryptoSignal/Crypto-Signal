...
class ExchangeInterface:
                ...
                if new_exchange:
                    ...
                    if "all_pairs" in exchange_config[exchange]:
                        if len(exchange_config[exchange]["all_pairs"]) > 0:
                            self.base_markets[
                                new_exchange.id
                            ] = exchange_config[exchange]["all_pairs"]
                        else:
    ...
    def get_exchange_markets(self, exchanges=[], markets=[]):
        ...
        for exchange in exchanges:
            exchange_markets[exchange] = self.exchanges[
                exchange
            ].load_markets()
            curr_markets = {
                k: v
                for k, v in exchange_markets[exchange].items()
                if v["active"] == True
            }
            ...
            if markets:
                ...
                if self.base_markets[exchange]:
                    if self.top_pairs and self.top_pairs > 0:
                        self.logger.info(
                            "Getting top %d pairs from %s in %s",
                            self.top_pairs,
                            str(self.base_markets[exchange]),
                            exchange,
                        )
                        all_markets = {
                            key: curr_markets[key]
                            for key in curr_markets
                            if curr_markets[key]["quote"]
                            in self.base_markets[exchange]
                        }
                        ...
                        self.logger.info(
                            "Getting all %s market pairs for %s "
