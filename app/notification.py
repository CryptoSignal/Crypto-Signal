...
class Notifier(IndicatorUtils):
    ...
    def notify_telegram(self, messages, chart_file):
        ...
        if self.enable_charts:
            if chart_file and os.path.exists(chart_file):
                ...
                self.logger.info(
                    "Chart file %s doesnt exist, sending text message.",
                    chart_file,
                )
                self.telegram_client.send_messages(formatted_messages)
        else:
                    ...
                    for indicator in new_analysis[exchange][market_pair][indicator_type]:
                        for index, analysis in enumerate(
                            ...
                            if latest_result["is_hot"] or latest_result["is_cold"]:
                                ...
                                if (
                                    "mute_cold" in analysis["config"]
                                    and analysis["config"]["mute_cold"] == True
                                    and latest_result["is_cold"] == True
                                ):
                                    self.logger.info(
                                        "Skiping cold notification for %s %s %s",
                                        market_pair,
                                        indicator,
                                        candle_period,
                                    )
    ...
    def plot_macd(self, ax, df, candle_period):
        ...
        ax.bar(
            x=_time,
            bottom=[0 for _ in macd_h.index],
            height=macd_h,
            width=bar_width,
            color="red",
            alpha=0.4,
    ...
    def plot_ichimoku(self, ax, df, historical_data, candle_period):
        ...
        ax.fill_between(
            ...
            leading_span_a,
            leading_span_b,
            where=leading_span_a > leading_span_b,
            facecolor="#008000",
            interpolate=True,
            alpha=0.25,
        )
        ax.fill_between(
            ...
            leading_span_a,
            leading_span_b,
            where=leading_span_b > leading_span_a,
            facecolor="#ff0000",
            interpolate=True,
            alpha=0.25,
        )
